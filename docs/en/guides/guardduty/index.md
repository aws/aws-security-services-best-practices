...
If you build your own tooling against the GuardDuty API in a large organization, be deliberate about which API you call in per-account loops. A pattern we see cause problems is a nightly job that walks every account and calls a describe-style operation such as `GetMembers` once per account. In an organization with hundreds or thousands of accounts, that volume of calls runs into `TooManyRequestsException` throttling, and the job either fails partway or takes far longer than it should.

The fix is to call [`ListMembers`](https://docs.aws.amazon.com/guardduty/latest/APIReference/API_ListMembers.html) with pagination, which returns member accounts in pages from a single call rather than requiring one call per account. This is a general principle worth applying across the AWS security services: when you need data for every account in an organization, reach for the paginated list operation before a per-account describe loop, and add retry with exponential backoff for the throttling you will still occasionally hit at scale. The result is fewer API calls, lower chance of throttling, and a job that scales with your organization instead of degrading as it grows.
...
There is a related detection worth knowing about even if you do not enable AI Protection. The `DefenseEvasion:IAMUser/BedrockLoggingDisabled` finding type alerts you when Bedrock model invocation logging is turned off, which is a common precursor to activity someone does not want recorded. In fact, foundational GuardDuty already contributes several AI-workload detections on its own by monitoring CloudTrail management events, such as unusual removal of Bedrock guardrails, changes to a model's training data source that could indicate data poisoning, and exfiltrated EC2 credentials used to call Bedrock or SageMaker AI APIs. AI Protection adds the data-event analysis on top of that baseline.

A point of confusion worth heading off: AI Protection is not a standalone service. Foundational GuardDuty has to be enabled in an account first, and AI Protection is then one of the optional protection plans you layer on. What you do not have to do is turn on everything else to get it. You can enable AI Protection by itself without enabling the other protection plans, so a common goal is to add just this one plan to an environment that already runs foundational GuardDuty. From the console, enable it from the AI Protection page under Protection plans rather than using the configure-all-capabilities action, which would switch on plans you did not intend. In an organization, the delegated administrator enables it for members from that same page, and you can do the same through the API or CLI by enabling only the AI Protection feature on the detector. This lets you keep AI Protection scoped to the accounts actually running Bedrock or SageMaker AI workloads rather than paying for it organization wide.
...
You can enable S3 Malware protection through the console, CLI, API, or through infrastructure as code such as [CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-guardduty-malwareprotectionplan.html) or [Terraform](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/guardduty_malware_protection_plan). Keep in mind that you enable this specifically for a bucket at a time and not across an entire account or accounts. Refer to the [deployment documentation](https://docs.aws.amazon.com/guardduty/latest/ug/enable-malware-protection-s3-bucket.html) for the exact steps but we have placed images below for you to get an idea of what the deployment looks like.
...
If you are new to GuardDuty the easiest way to view/filter your findings is via the Summary tab. The Summary dashboard displays an aggregated view of GuardDuty findings generated in the current AWS Region. You can choose to view the Last 2 days (default), Last 7 days or Last 30 days. This dashboard provides 6 widgets, 3 of which include filter capabilities to customize your view.
...
You can see an example EventBridge rule in the [GuardDuty documentation](https://docs.aws.amazon.com/guardduty/latest/ug/monitor-with-eventbridge-s3-malware-protection.html). From here you will need to configure a Lambda function to use that EventBridge rule as a trigger and then moves the file based on your needs. Example Python code to do this is shown below. Keep in mind this is example code and should be altered to fit your use case.
...
A note on estimating spend during the free trial, since this trips people up. The GuardDuty usage view reports consumption in the units GuardDuty actually meters, such as GB of logs analyzed and event counts, rather than a projected dollar figure. If you are looking for "what will this cost me," those raw metrics are the input, not the answer. Combine them with the rates on the [GuardDuty pricing page](https://aws.amazon.com/guardduty/pricing/) to translate usage into an estimate, and account for GuardDuty's tiered volume discounts, which make the effective per-GB rate fall as monthly volume rises. If you also run other security services, the [Security Hub Cost Estimator](../security-hub/index.md#comparing-individual-service-and-security-hub-pricing) is the more direct way to get a consolidated dollar estimate, because it pulls your actual usage from Cost Explorer and applies current rates for you.
